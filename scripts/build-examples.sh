...
then
  ...
  then
    ...
    ls examples/specs/*.vl.json | parallel --eta --no-notice --plus --halt 1 "bin/vl2vg -p {} > examples/compiled/{/..}.vg.json && node_modules/.bin/vg2svg -t examples/compiled/{/..}.vg.json examples/compiled/{/..}.svg -b ."
  else
    ...
    for file in examples/specs/*.vl.json; do
      ...
      node_modules/.bin/vg2svg -t $dir/$name.vg.json $dir/$name.svg -b .
    done
  fi
else
  for name in "$@"
  do
    ...
    node_modules/vega/bin/vg2svg -t examples/compiled/$name.vg.json > examples/compiled/$name.svg -b .
  done
fi
